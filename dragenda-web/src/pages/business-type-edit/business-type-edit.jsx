...
import "../business-type-add/business-type-add.css";

function BusinessTypeEdit() {
  ...
  useEffect(() => {
    // Carregar os dados do tipo de negócio
    async function loadBusinessType() {
      try {
        const response = await api.get(`/admin/business-types/${id}`);
        if (response.data) {
        ...
        if (error.response?.status === 401) {
          ...
          setError("Tipo de negócio não encontrado");
        } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await api.put(`/admin/business-types/${id}`, {
        name,
        description,
        icon
      });
      ...
      if (response.data) {
        setSuccessMsg("Tipo de negócio atualizado com sucesso!");
      ...
      setError(error.response?.data?.error || "Erro ao atualizar tipo de negócio.");
    }
  };
          ...
          <div className="col-lg-8">
            <div className="card">
              <div className="card-header bg-primary text-white">
                <h4 className="mb-0">Editar Tipo de Negócio</h4>
              </div>
